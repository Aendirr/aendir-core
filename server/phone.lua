...
RegisterNetEvent('aendir:server:BuySimCard', function()
    ...
    if Player.Functions.RemoveMoney('cash', Config.SimCardPrice) then
        local phoneNumber = GeneratePhoneNumber()
        Player.Functions.AddItem('sim_card', 1, false, {
            number = phoneNumber
        })
        TriggerClientEvent('aendir:client:Notification', src, 'success', 'SIM kart satın aldınız! Numaranız: ' .. phoneNumber)
    else
...
RegisterNetEvent('aendir:server:BuyPhone', function()
    ...
    if Player.Functions.RemoveMoney('cash', Config.PhonePrice) then
